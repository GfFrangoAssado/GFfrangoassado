...
function loadLoyaltyCustomers() {
    ...
    onValue(usersRef, async (snapshot) => {
        ...
        console.log("Carregando clientes para fidelidade...");
        ...
        for (const userId in snapshot.val()) {
            ...
            let totalSpent = 0;
            let actualCompletedCount = 0; // Contador real baseado nos pedidos entregues
            const userOrders = userSnapshot.child('pedidos').val();
            if (userOrders) {
                for (const orderId in userOrders) {
                    const order = userOrders[orderId];
            ...
            console.log(`Cliente ${userId}: completedOrders=${completedOrders}, actualCompletedCount=${actualCompletedCount}, totalSpent=${totalSpent}`);

            // Usa o maior valor entre o contador salvo e o contador real
            const finalOrderCount = Math.max(completedOrders, actualCompletedCount);

            if (finalOrderCount >= 5) {
                customers[userId] = {
        ...
        console.log(`Clientes fidelidade encontrados: ${sortedCustomers.length}`);
...
// ==========================================================
// LÓGICA DE CUPOM DE DESCONTO (ADMIN)
// ==========================================================
const cupomForm = document.getElementById('cupomForm');
const cupomCodigoInput = document.getElementById('cupom-codigo');
const cupomDescricaoInput = document.getElementById('cupom-descricao');
const cupomProdutosInput = document.getElementById('cupom-produtos');
const cupomAtivoSelect = document.getElementById('cupom-ativo');
const cupomStatusDiv = document.getElementById('cupom-status');

// Função para gerar código aleatório de cupom (6 caracteres, letras e números)
function gerarCodigoCupom(length = 6) {
...
// Carregar cupom existente ao abrir a seção
function carregarCupomDesconto() {
    const cupomRef = ref(db, 'config/cupom');
    onValue(cupomRef, (snapshot) => {
        const data = snapshot.val();
        if (data) {
            cupomCodigoInput.value = data.codigo || '';
            cupomDescricaoInput.value = data.descricao || '';
            cupomProdutosInput.value = (data.produtos || []).join(',');
            cupomAtivoSelect.value = data.ativo ? 'true' : 'false';
            cupomStatusDiv.textContent = data.ativo ? `Cupom ativo: ${data.codigo}` : 'Cupom inativo';
        } else {
            cupomCodigoInput.value = '';
            cupomDescricaoInput.value = '';
            cupomProdutosInput.value = '';
            cupomAtivoSelect.value = 'false';
            cupomStatusDiv.textContent = 'Nenhum cupom cadastrado.';
        }
    }); // Removido { onlyOnce: true }
}

// Salvar cupom no Firebase ao submeter o formulário
if (cupomForm) {
    cupomForm.onsubmit = async (e) => {
        e.preventDefault();
        let codigo = cupomCodigoInput.value.trim();
        if (!codigo) {
            codigo = gerarCodigoCupom();
            cupomCodigoInput.value = codigo; // Preenche no campo para feedback visual
        }
        const descricao = cupomDescricaoInput.value.trim();
        const produtos = cupomProdutosInput.value.split(',').map(p => p.trim()).filter(Boolean);
        const ativo = cupomAtivoSelect.value === 'true';
        if (!descricao || produtos.length === 0) {
            alert('Preencha todos os campos obrigatórios.');
            return;
        }
        try {
            await set(ref(db, 'config/cupom'), {
                codigo,
                descricao,
                produtos,
                ativo
            });
            cupomStatusDiv.textContent = ativo ? `Cupom ativo: ${codigo}` : 'Cupom inativo';
    ...
    // Carrega cupom ao abrir a página
    carregarCupomDesconto();
}